...
import { IBaseDeviceMandatoryRaw, IBaseDeviceRaw } from './IBaseDeviceRaw';
...
export class BaseDevice extends _ObjectSubSite {
    static debug = createDebugger('Device');
    constructor(config: IObjectSubSiteConfig, props: Partial<IBaseDeviceRaw> & { mac: string }) {
        super(config);
        ...
        this.debug = BaseDevice.debug.extend(this.mac);
        this.import(props);
    }

    public import(props: Partial<IBaseDeviceRaw> & IBaseDeviceMandatoryRaw): this {
        ...
        if (!Validate.isUndefined(props.type)) {
            this.type = props.type as EDeviceType;
        }
     ...
     */
    public setLedOverride(props: ILedOverrideProps): unknown {
        const payload: {
    ...
    public updateDevice(payload: Partial<IBaseDeviceRaw>): Promise<this> {
        return this._updateDevice(payload);
    }

    protected async _updateDevice(payload: unknown): Promise<this> {
        const res = (
            await this.instance.put<IUnifiResponseEnveloppe<Array<IBaseDeviceRaw>>>('/rest/device/:_id', payload, {
                urlParams: {
                    _id: this._id
                }
            })
