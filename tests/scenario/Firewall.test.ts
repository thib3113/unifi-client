import Controller, { FWGroup, FWRule, IFWV4RuleRaw, IFWV6RuleRaw, Site } from '../../src';
import { getLoggedSite } from './_scripts/common';
import nock from 'nock';
import { IObjectSubSiteConfig } from '../../src';
...
describe('Firewall - UnifiOs', () => {
    ...
    describe('Rules V6', () => {
        it('should return the firewall rules', async () => {
            await nock.back(`${PREFIX}get-rulesv6.json`).then(async ({ nockDone }) => {
                const [rule] = await site.firewall.getRules();
        ...
        it('should crud firewall rule', async () => {
            let rule: FWRule;
            let rule2: FWRule;
            const ruleParams: Omit<IFWV6RuleRaw, '_id' | 'site_id'> & Partial<{ _id: string; site_id: string }> = {
                action: 'accept',
                enabled: true,
                dst_firewallgroup_ids: [],
                ipsec: '',
            ...
            await nock.back(`${PREFIX}rulev6-create.json`).then(async ({ nockDone }) => {
                rule = await site.firewall.createRule(ruleParams);
            ...
            await nock.back(`${PREFIX}rulev6-get.json`).then(async ({ nockDone }) => {
                const getRule = await site.firewall.getRule(rule._id);
            ...
            await nock.back(`${PREFIX}rulev6-edit.json`).then(async ({ nockDone }) => {
                rule.name = 'testRule2';
                await rule.save();
            ...
            await nock.back(`${PREFIX}rulev6-delete.json`).then(async ({ nockDone }) => {
                await rule.delete();
