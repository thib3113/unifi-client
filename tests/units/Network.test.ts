...
describe('Network', () => {
    ...
    describe('construct', () => {
        describe('network', () => {
            it('should construct Network', () => {
                const network = new Network({ controller, site }, { name: 'aaaaa' });
            });
            it('should construct Network with enabled', () => {
                const network = new Network({ controller, site }, { name: 'aaaaa', enabled: true });
                expect(network.enabled).toBe(true);
            });
            it('should refuse a Network without name', () => {
                expect.assertions(3);
                try {
                    // @ts-ignore
                    new Network({ controller, site }, {});
                } catch (e) {
                    expect(e).toBeInstanceOf(ClientError);
                    expect(e.message).toBe('name is mandatory for a site.');
    ...
    describe('functions', () => {
        let network: Network;
        const networkManagerMock = jest.fn();
        beforeEach(() => {
            network = new Network({ controller, site }, { name: 'Test Network' });

            // @ts-ignore
            network.site = {
                devManager: networkManagerMock,
        ...
        describe('save', () => {
            ...
            it('should put the data', async () => {
                ...
                network._id = 'aaaaaaa';
                network.name = 'UDMPRO2';
                network.enabled = true;
                expect(await network.save()).toBe(network);

                expect(importMock).toBeCalledWith({ _id: 'aaaaaaa', name: 'UDMPRO2' });
                expect(mockedAxios.put).toHaveBeenCalledWith(
                    '/rest/networkconf/:_id',
                    {
                        name: 'UDMPRO2',
                        enabled: true
                    },
                    { urlParams: { _id: 'aaaaaaa' } }
                );
            });
        });
    });
